...
public class InvokeParticleAnimation : Command
{
 ...
 {
  public PSMaterialProperties (int min, int max, int ratio)
  {
   this.min = min;
   this.max = max;
   this.numParticles = ratio;
  }
 ...
 public InvokeParticleAnimation (Actor actor, string name) : base (actor)
 {
  ...
  if (m_ProperyMap == null) {
   ...
   m_ProperyMap.Add ("HP_down", new PSMaterialProperties (5, 10, 10));
   m_ProperyMap.Add ("HP_up", new PSMaterialProperties (30, 40, 10));
   m_ProperyMap.Add ("Exclamation", new PSMaterialProperties (100, 1000, 100));
...
//  ps.loop = false;
  ...
  GameObject.Destroy (EmitterObject, psprops.max);
